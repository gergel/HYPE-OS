# Lara — elfogadási ellenőrzőlista
...
Backend Lara tesztek: **25 zöld** (`pytest tests/test_admin_agent_*.py`).
...
| 17 | Projektkód több projektet fog össze; kiadások pontosan egyszer összesülnek | ✅ | Lara a MEGLÉVŐ `szamla_erkeztetes.jovahagy`-on át rögzít (nem duplikál); invariáns a pénzügyi szolgáltatásban |
| 18 | Több deviza/áfa/kerekítés/stornó helyesen vagy emberhez | 🟡 | A meglévő érkeztető kezeli ezeket; Lara a `felosztas`/összeg-egyezést a szolgáltatásra bízza, hiánynál NEEDS_INFO |
...
- **Valós modellhívás (Gemini):** a kód bekötve (számla-átnézés, TIG/szerződés-
  ...
  ellenőrzött: `GEMINI_API_KEY` beállítása után egy számla „Lara" gombbal
  és egy TIG-feladat „Tervezet készítése" gombbal ellenőrizendő — „Lara
