# Lara – architektúra és bekötési térkép
...
| Terület | Ami a repóban VAN | Lara ezt használja |
|---|---|---|
...
- **Multi-tenancy / `org_id`:** a HYPE OS **egyszervezetes** — nincs `org_id`
  a modellekben, nincs több-tenant elkülönítés. Ezért Lara sem vezet
...
2. Lara **mindig a meglévő pénzügyi szolgáltatáson keresztül** dolgozzon
...
3. **Kiadás akkor számít „elköltöttnek"** a kimutatásba, ha ki van fizetve
   (`kesz`) – lásd a `fizetes_datuma`/`kiadas_datuma` összevonást. Lara
   ezt nem kerülheti meg.
4. Lara **nem** végez könyvelési feladást, kifizetettre állítást
...
- `backend/app/admin_agent/` – enumok, policy engine, settings-szolgáltatás,
  (később) Lara-mag, ingest, tanulás.
