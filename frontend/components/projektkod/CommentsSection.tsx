...
import type { ProjectCodeComment } from "@/lib/api";
...
/** Chat-szerű hozzászólás-lista egy Project Code oldal alján - ugyanaz a
 * minta, mint az Utómunkánál (lásd components/deliverable/CommentsSection.tsx):
 * a résztvevők névvel beszélgetnek, és "@" beírásával egymást is meg tudják
 * taggelni. Lapos lista, nem szálakba rendezett válaszok. */
export function CommentsSection({
  ...
  initialComments: ProjectCodeComment[];
  ...
  // háttérfigyelő szól, ha ennek a projektkódnak a hozzászólásai változtak,
  // és csak akkor kérjük le újra a listát. A félig megírt saját szöveg (body)
  // érintetlen marad, mert az külön állapot.
  useLiveTopic(`projectCodeComments:${projectCodeId}`, () => {
    authFetch(`/api/v1/project-codes/${projectCodeId}/comments`)
      .then((res) => (res.ok ? res.json() : null))
      .then((fresh: ProjectCodeComment[] | null) => fresh && setComments(fresh))
      .catch(() => {});
  });
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      const res = await authFetch(`/api/v1/project-codes/${projectCodeId}/comments`, {
        method: "POST",
      ...
      const created: ProjectCodeComment = await res.json();
