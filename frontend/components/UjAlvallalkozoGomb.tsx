...
/** ÚJ ALVÁLLALKOZÓ felvétele helyben (a felhasználó kérése): amikor az
 * utókövetéshez vezetünk fel külsős kiadást, és az illető még nincs a
 * munkatárs-listában, ne kelljen átmenni a Csapat oldalra - itt egyben
 * megadható minden adata, a vállalkozási (papírozási) mezőkkel együtt,
 * amikből a szerződés és a TIG előtölt (lásd backend
 * services/szamlazo.SzamlazoFel és schemas/employee.EmployeeCreate).
 *
 * A mentés után a lap frissül, és az új ember azonnal választható az
 * "Alvállalkozó (ha van)" legördülőben. */
export function UjAlvallalkozoGomb() {
  const router = useRouter();
  const [nyitva, setNyitva] = useState(false);
  const [busy, setBusy] = useState(false);
  const [hiba, setHiba] = useState<string | null>(null);
  const [mentve, setMentve] = useState<string | null>(null);
  ...
  async function ment() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setHiba(`Sikertelen mentés: ${detail?.detail ?? res.status}`);
        return;
      }
      setMentve(adatok.full_name.trim());
      setNyitva(false);
      // A frissítés után az új ember ott van a legördülőben - a QuickCreateForm
      // nyitott állapota és beírt értékei megmaradnak (kliens-állapot).
      router.refresh();
    } catch (err) {
  ...
  return (
    <>
      <button
        ...
        className="mb-2 inline-flex items-center gap-1.5 text-[13px] text-text-accent hover:underline"
      >
        <UserPlus size={14} />
        Új alvállalkozó felvétele
      </button>
      {mentve && (
        <span className="mb-2 ml-3 text-[12.5px] text-text-success">
          {mentve} felvéve – már választható a listában.
        </span>
      )}
      {nyitva && (
        <ModalReteg onClose={() => setNyitva(false)}>
          ...
          >
            ...
            <p className="mb-4 text-[13px] text-text-secondary">
              Külsős munkatársként kerül a rendszerbe. A vállalkozási adatokból tölt elő a szerződése és a TIG-je –
              amit most nem tudsz, később a Csapat oldalán pótolható.
            </p>
